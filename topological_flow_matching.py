...
class TopologicalFMGFlowNet(FMGFlowNet):
    r"""Topological Flow Matching GFlowNet, with edge flow estimator. This GFlowNet learns backwards from the 
    terminating states to the non-terminating states.
    ...
    def flow_matching_loss(
        ...
        """Computes the FM for the terminating states for every action.
        ...
        incoming_log_flows = torch.full_like(
            states.backward_masks, float(0), dtype=torch.float
        )
        outgoing_log_flows = torch.full_like(
            states.forward_masks, float(0), dtype=torch.float
        )
    ...
    def loss(
        ...
        trajectories."""
        ( # no need for intermediary states in topological flow matching
            _,
            terminating_states,
            _,
        ...
        # do reward matching for exit action
