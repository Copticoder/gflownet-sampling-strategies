...
class GrowingTriangleSampler(Sampler):
    ...
    def sample_actions(
        ...
        **policy_kwargs: Any,
        ...
        if self.topological_type != "normal" and self.triangle_counter <= env.n:
            # allow only the exit action for states that are at frontier 
            frontier = self.triangle_counter if self.topological_type == "small_then_large" else env.n - self.triangle_counter
